...
PHP_METHOD(SnailHttp_Uri, filterPort) {
	...
	if (!(_0)) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_integer", NULL, 38, port);
		zephir_check_call_status();
		_2 = zephir_is_true(_1);
		if (_2) {
...
PHP_METHOD(SnailHttp_Uri, filterPath) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("preg_replace_callback", NULL, 39, _1, _0, path);
...
PHP_METHOD(SnailHttp_Uri, filterQuery) {
	...
	ZEPHIR_INIT_VAR(_1);
	ZVAL_STRING(_1, "/(?:[^a-zA-Z0-9_\\-\\.~!\\$&\\'\\(\\)\\*\\+,;=%:@\\/\\?]+|%(?![A-Fa-f0-9]{2}))/", ZEPHIR_TEMP_PARAM_COPY);
	ZEPHIR_RETURN_CALL_FUNCTION("preg_replace_callback", NULL, 39, _1, _0, query);
	zephir_check_temp_parameter(_1);
	zephir_check_call_status();
	RETURN_MM();
